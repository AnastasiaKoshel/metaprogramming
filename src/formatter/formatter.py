import re
from formatter import regex_consts as regex
from formatter import level_of_idents
...
class Formatter:
    ...
    def __init__(self, config, input_filename, output_filename):
        self.config = config
        self.level_of_idents = level_of_idents.LevelOfIdents(config)
    ...
    def write_line(self):
        self.current_line = self.level_of_idents.format_line(self.current_line)
    ...
    def process_line(self, line_num, line):
        self.process_IF_WHILE_SWITCH(line_num, line)
        ...
        self.write_line()
        self.level_of_idents.iterate()
        # return line

    def process_IF_WHILE_SWITCH(self, line_num, line):
        """
    ...
    def process_case(self, line_num, line):
        ...
        if search is not None:
            ...
            if ':' in prefix:
                ...
                # 'case ...: ...\n'
                    ...
                    self.current_line = prefix
                    self.remainder = line
                    self.level_of_idents.found_case()
        ...
        if search is not None:
            ...
            # 'break;'
            if ';' in prefix:
                self.level_of_idents.found_break()
                self.current_line = prefix
                self.remainder = line
    ...
    def process_closing_brace(self, line_num, line):
        ...
        if brace_search is not None:
            ...
            self.level_of_idents.ident_ended()
    ...
    def handle_curly_brace_line(self, prefix, line):
        ...
        if semicolon_search is not None:
            ...
            self.current_line += s
            self.remainder = line
            self.level_of_idents.found_simple_operator(prefix == "")
            return
        ...
        if brace_search is not None:
            ...
            self.current_line += s
            self.remainder = line
            self.level_of_idents.new_ident_found()
